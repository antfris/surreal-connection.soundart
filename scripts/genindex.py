...
for folder_name in sorted(os.listdir(root_folder)):
    ...
    if os.path.isdir(folder_path):
        ...
        if os.path.exists(mp3_path):
            ...
            rec_date_str = str(audiofile.tag.recording_date)
            if len(rec_date_str) == 4:  # only year is present
                rec_date_str += "-01-01"
            
            recording_date = datetime.datetime.strptime(rec_date_str, '%Y-%m-%d').date()
